...
| 📄 **리포트 스튜디오** | 자연어 요청 → **선언적 Report Spec(DSL)** → 관세청 데이터 리졸브 → **인터랙티브 리포트 자동 조립**([app/report.html](app/report.html)). LLM은 스펙만 설계하고 수치는 엔진이 계산(환각 차단) — [설계 문서](docs/GENERATIVE_REPORTS.md) |
...
데모 데이터는 관세청 공개 통계에 앵커링한 값이며 개인·기업 식별정보를 포함하지 않습니다.
