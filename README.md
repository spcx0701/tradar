...
<p align="center"><strong>관세청 수출입통계와 산업부 K-SURE·KOTRA 데이터, AI로 전 수출산업의 품목·국가별 시장을 스코어링·예측하는 수출 데이터 분석 플랫폼.</strong></p>
...
<p align="center">
  <a href="https://tradar.onrender.com/"><strong>플랫폼 열기 →</strong></a> ·
  <a href="https://www.data.go.kr/data/15100475/openapi.do"><strong>관세청 데이터</strong></a> ·
  <a href="https://www.data.go.kr/data/15140201/openapi.do"><strong>산업부 데이터</strong></a>
</p>

<p align="center">
  <a href="https://play.google.com/store/apps/details?id=kr.tradewind.app"><img alt="Get it on Google Play" src="https://play.google.com/intl/en_us/badges/static/images/badges/en_badge_web_generic.png" align="center" height="64"></a>
...
| 📄 **리포트 스튜디오** | 자연어 요청 → **선언적 Report Spec(DSL)** → 관세청·산업부 데이터 리졸브 → **인터랙티브 리포트 자동 조립**([app/report.html](app/report.html)). LLM은 스펙만 설계하고 수치는 엔진이 계산(환각 차단) — [설계 문서](docs/GENERATIVE_REPORTS.md) |

## 데이터 (관세청·산업부 공공데이터 연동)
...
- **산업통상부 공공데이터 레이어** — [scripts/sync_motie_data.py](scripts/sync_motie_data.py)([server/motie_client.py](server/motie_client.py))가 산업부 산하기관 데이터를 `app/data/motie.js`(`window.TRADAR_MOTIE`)로 굽는다: 한국무역보험공사 [국별신용등급](https://www.data.go.kr/data/15140201/openapi.do)·[국가별 업종별 위험지수](https://www.data.go.kr/data/15132755/openapi.do), 대한무역투자진흥공사(KOTRA) [해외시장뉴스](https://www.data.go.kr/data/15034831/openapi.do)·[수입규제품목 정보](https://www.data.go.kr/data/15088467/openapi.do). 마켓맵 **K-SURE 리스크 레이어**, 무역상대국 **K-SURE 등급·對韓 수입규제** 컬럼, 뉴스·경보에 연동. 키 미설정 시 공표치 앵커.
...
> 관세청 수출입실적과 산업부 K-SURE·KOTRA 데이터를 함께 쓰며, 국산 AI 엔진은 예측·조기경보·근거기반 상담을 담당합니다.

## 작동 방식

```
관세청 품목별 국가별 수출입실적 + 산업부 K-SURE·KOTRA 데이터
...
python scripts/sync_motie_data.py       # 산업부(K-SURE·KOTRA) 데이터 → app/data/motie.js
python scripts/serve.py                 # 플랫폼: http://localhost:5183
```

English repository README: [README.en.md](README.en.md).
...
| `GET/POST /api/admin/llm-provider` | 관리자 토큰, `{"provider": "openrouter-solar-free"}` | 서버 LLM provider 상태 조회·선택 |
| `POST /api/report` | `{"question": "..."}` | 자연어 → Report Spec 플랜 → 데이터 리졸브(리포트 스튜디오) |
| `POST /api/report/render` | `{"spec": {...}}` | 스펙 직접 리졸브 — UI 상호작용 라운드트립 |
| `GET /api/report/schema` | 없음 | Report Spec DSL의 JSON 스키마(계약) |

관리자 UI는 `/admin.html`에서 제공하며, `TW_ADMIN_TOKEN`으로 보호된 서버 provider 선택 API만 호출합니다.
...
app/
  ...
  vendor/            React 18 · ECharts 5 · liquid-glass-react 1.1.1(MIT, 리퀴드 글래스 UI) — 오프라인용 로컬 벤더링
...
별도 표기가 없는 한 프로젝트 코드, 문서, Android 리소스, 프로젝트 소유 이미지 자산은
[GNU Affero General Public License v3.0 or later](LICENSE) (AGPL-3.0-or-later)로 배포됩니다.
이 소프트웨어를 수정해 네트워크로 서비스하는 경우, 그 이용자에게 수정본의 소스 코드를 제공해야 합니다.
Android 모바일 클라이언트에는 앱스토어 배포를 위한 AGPL 7조 추가 허가가 적용되며, 서버와 웹/PWA
클라이언트는 이 예외 대상이 아닙니다. 자세한 내용은 [LICENSE](LICENSE)와 [NOTICE](NOTICE)를 참고하세요.
데모 데이터는 관세청·산업부 공개 통계에 앵커링한 값이며 개인·기업 식별정보를 포함하지 않습니다.
