...
| Coverage upload | Codecov | `CI / test` uploads `coverage.xml` with GitHub OIDC. Display the coverage badge only after Codecov returns a concrete percentage instead of `unknown`. |
